...
async function verifyUser(req, res, next) {
  if (
    typeof req.user_id === 'undefined' ||
    typeof req.body.user_id === 'undefined' ||
    req.user_id !== parseInt(req.body.user_id, 10)
  ) {
...
async function updatePassword(req, res, next) {
  await User.findOne({
    ...
    if (user === null) res.status(403).send({ message: 'User not found.' });

    // Compare hashed password from DB and from Request
    bcrypt.compare(req.body.old_password, user.password, (err, result) => {
      if (result === true) next();
      else res.status(403).send({ message: 'Password does not match' });
    });
  });
}
